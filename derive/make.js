...
((stream) => {
	const out = createWriteStream(new URL('./log.txt', out_dir).pathname);
	const old = stream.write.bind(stream);
	stream.write = (...a) => {
		old(...a);
		out.write(...a);
	};
...
const ignored = new Set(IDNA.ignored);
const valid = new Set(IDNA.valid);
const mapped = new Map(IDNA.mapped);
const valid_emoji = new Map();
const disabled_emoji = new Map();

// this should be safe by construction
// a character should only be in one of the sets
function disallow_char(cp) {
	const ys = mapped.get(cp);
	if (ys) {
		mapped.delete(cp);
...
const emoji_data = UNICODE.read_emoji_data();
const emoji_map = new Map(emoji_data.Emoji.map(x => [x.cp, x]));

print_section(`Demote Emoji to Characters`);
for (const cp of EMOJI_DEMOTED) {
	const info = emoji_map.get(cp);
...
for (const cp of EMOJI_DISABLED) {
	const info = emoji_map.get(cp);
	if (!info) throw new Error(`Expected emoji: ${PRINTER.desc_for_cp(cp)}`);
	if (info.used) throw new Error(`Duplicate: ${PRINTER.desc_for_cp(cp)}`);
	info.used = true;
...
for (const seq of emoji_data.Emoji_Presentation) {
	const info = emoji_map.get(seq.cp);
	if (!info) throw new Error(`Expected emoji: ${PRINTER.desc_for_emoji(seq)}`);	
	if (info.used) continue;
	info.used = true;
...
for (const info of EMOJI_SEQ_WHITELIST) {
	register_emoji({cps: parse_cp_sequence(info.hex), type: 'Whitelisted', name: info.name});
}

print_section('Blacklist Emoji');
for (const def of EMOJI_SEQ_BLACKLIST) {
	const cps = Number.isInteger(def) ? [def] : parse_cp_sequence(def);
	const key = String.fromCodePoint(...cps);
	const info = valid_emoji.get(key);
...
print_section('Assign Emoji Groups');
{
	...
	for (const [k, v] of valid_emoji)    map.set(strip_fe0f(k), v);
	for (const [k, v] of disabled_emoji) map.set(strip_fe0f(k), v);
	for (const [key, tests] of group_by(UNICODE.read_emoji_test(), x => strip_fe0f(String.fromCodePoint(...x.cps)))) {
		const info = map.get(key);
		if (!info) continue;
		const test = tests.find(x => x.type === 'fully-qualified' || x.type === 'component'); // exclusive
...
for (const [x, ys] of CHARS_MAPPED) {
	const old = mapped.get(x);
...
for (const cp of CHARS_DISALLOWED) {
...
for (const [cp, abbrs] of SCRIPT_EXTENSIONS) {
	try {
		...
		const scripts = [...new Set(abbrs.sort().map(x => UNICODE.require_script(x)))]; // sorted
		const char = UNICODE.require_char(cp);
...
const untested_set = new Set(UNICODE.script_map.values());	
...
let script_groups = [];
for (const config of SCRIPT_GROUPS) {
	const {name, test, rest = [], extra = [], cm = false} = config;
	try {
		if (!name) throw new Error(`Expected name`);
		if (!Array.isArray(test) || !test.length) throw new Error(`Expected test`);
		const test_set = new Set();
		for (const abbr of test) {
			const script = UNICODE.require_script(abbr);
		...
		const extra_set = new Set(extra.flat(Infinity).flatMap(x => typeof x === 'string' ? explode_cp(x) : x));
		const group = new ScriptGroup(name, test_set, rest_set, extra_set, cm);
...
for (const abbr of DISALLOWED_SCRIPTS) {
	const script = UNICODE.require_script(abbr);
	if (!untested_set.delete(script)) {
...
for (const abbr of RESTRICTED_SCRIPTS) {
	const script = UNICODE.require_script(abbr);
	if (!untested_set.delete(script)) {
		throw new Error(`Missing restricted script: ${script.description}`);
	}
	const group = new ScriptGroup(/*script.name*/script.abbr, new Set([script]), new Set(), new Set(), false);
...
for (const cp of valid) {
	const char = UNICODE.require_char(cp);
	const scripts = char.get_extended();
	for (const g of script_groups) {
...
const cm_whitelist = new Set();
for (const form of CM_WHITELIST) {
	const cps = NF.nfc(explode_cp(form));
	try {
		const key = String.fromCodePoint(...cps); 
		...
		const [nfc_cp0, ...cms] = cps;
		const base_char = UNICODE.require_char(nfc_cp0);
		...
		const groups = script_groups.filter(g => g.valid_set.has(nfc_cp0));
		if (!groups.length) {
...
for (const g of script_groups) {
	...
	if (g.cm_whitelisted) {
		...
		// remove unreachable
		const cm = new Set([...g.cm_map.values()].flat(Infinity)); 
		for (const cp of g.valid_set) {
		...
		for (const cp of g.valid_set) {
			const cps = NF.nfd([cp]);
			const nsm = cps.filter(x => NSM.has(x));
...
let wholes_list = [];
const confused_union = new Set();
script_groups.forEach(g => { g.parts = g.compute_parts(); }); // cache
for (const [target, ...defs0] of CONFUSE_GROUPS) {
	const defs = [];
	...
	const whole = new Whole(target, defs, defs.filter(def => def.type === CONFUSE_TYPE_VALID).map(def => def.cp));
	wholes_list.push(whole);

	const unresolved = {};
	for (const g of union) {
		const confuse = defs.filter(x => x.groups.has(g));
		const decided = confuse.filter(x => x.type); 
		
		// if theres any decisions, use that
		if (decided.length) {
			for (const def of confuse) {
				switch (def.type) {
		...
		// keep track of it
		const key = confuse.map(def => def.cp).sort().join(); // meh
	...
	for (const [joined, gs] of Object.entries(unresolved)) {
		const cps = joined.split(',').map(s => parseInt(s));
...
const parts_union = new Set(script_groups.flatMap(g => [...g.compute_parts()]));
for (const cp of valid) {
...
script_groups = script_groups.filter(g => {
	...
	wholes_list = wholes_list.filter(w => {
		for (const [cp, set] of w.map) {
			if (set.delete(g) && !set.size) {
				w.map.delete(cp);
			}
		}
		return w.map.size;
...
for (const cp0 of valid) {
	for (const cp of NF.inner_parts(cp0)) {
		if (!valid.has(cp)) {
...
const escaped = new Set(CHARS_ESCAPE);
...
for (const cp of UNICODE.get_noncharacter_cps()) {
	if (valid.has(cp)) {
...
const nonzero_cc = new Set([...UNICODE.char_map.values()].filter(x => x.cc).map(x => x.cp)); 
for (const info of valid_emoji.values()) {
	const {cps} = info;
...
for (const g of script_groups) {
	...
	const upper = g.name.slice(0, 1);
	const lower = g.name.slice(1);
...
const valid_union = new Set();
const multi_group = new Set();
for (const g of script_groups) {
	for (const cp of g.valid_set) {
...
const valid_unique = new Set([...valid_union].filter(cp => !multi_group.has(cp)));
...
for (const cp of valid_unique) {
	const gs = script_groups.filter(g => g.valid_set.has(cp));
	if (!gs.length) throw new Error('bug');
	if (gs.length === 1) continue;
	const wl = gs[0].cm_whitelisted;
...
const confused_whole = wholes_list.flatMap(w => w.map.keys());
const confused_valid = wholes_list.flatMap(w => w.valid); 
...
for (const g of script_groups) {
	const {name, test_script_set, valid_set, restricted} = g;
	//g.parts = g.compute_parts(); // cache again
	let cm;
	if (g.cm_whitelisted) {
		cm = [];
		for (const [cp, seqs] of g.cm_map) {
	...
	const primary = [];
	const secondary = [];
	for (const cp of sorted(valid_set)) {
...
const ranks = UNICODE.combining_ranks();
const nfc_qc = UNICODE.read_nf_props().NFC_QC.map(x => x[0]);
const nfc_check0 = [...new Set([ranks, nfc_qc].flat(Infinity))];
const nfc_check = nfc_check0.filter(cp => parts_union.has(cp));
console.log(`Optimized: ${nfc_check0.length} => ${nfc_check.length}`);

print_section('Compute NSM from Groups w/o CM Whitelist');
const nsms = script_groups.filter(g => !g.cm_whitelisted).map(g => [...g.compute_parts()].filter(cp => NSM.has(cp)));
const nsm = new Set(nsms.flat());
...
function write_json(name, json, indent) {
	const file = new URL(name, out_dir);
	const str = JSON.stringify(json, null, indent ? '\t' : null); // 20240123: added optional indent for better diffs
	try {
		...
		deepEqual(...[readFileSync(file), str].map(x => {
			...
			const {created, ...rest} = json; // remove dated keys
			return rest;
		}));
...
write_json('emoji-info.json', [...valid_emoji.values(), ...disabled_emoji.values()].map(info => {
	const {cp, same, cps, used, ...rest} = info;
