...
function set_complement(set0, ...sets) {
...
function ens_normalize(name) { 
	...
	let labels = tokenized_idna(explode_cp(name), EMOJI_PARSER, cp => {
		...
		let text = tokens.reduce((a, {v}) => {
			if (v) {
				a.push(...nfc$1(v)); // required for Context/Bidi
...
export { BUILT, IDNA, UNICODE, VERSION, compare_arrays, ens_normalize, ens_tokenize, escape_name_for_html, escape_unicode, explode_cp, group_by, hex_cp, indices_of, is_printable_ascii, map_values, nfc$1 as nfc, nfc as nfc0, nfd$1 as nfd, nfd as nfd0, parse_cp, parse_cp_multi_ranges, parse_cp_range, parse_cp_sequence, puny_decode, quote_cp, random_bytes, random_choice, set_complement, set_intersect, set_union, split_ascending, split_between, split_linear, split_on, take_from, tally, tokenized_idna };
